...
- Docker Compose 管理本地 PostgreSQL，种子脚本提供演示账号和一篇已发布的真实排障文档。

第一位注册用户会成为 MVP 审核员，后续用户是普通贡献者。未通过审核的新条目和修改不会覆盖公开内容。
...
`seed` 会先完成建表，再幂等创建本地演示审核员、贡献者和一篇已发布的真实排障文章。演示账号由 `backend/.env` 中的 `APP_SEED_*` 配置；只需要建表时可改用 `python -m app.db.bootstrap`。上传图片保存在 `APP_UPLOAD_DIR` 指定的本地目录，生产环境应替换为持久化文件或对象存储。
